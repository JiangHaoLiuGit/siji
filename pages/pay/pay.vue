	...
	<view class="pay">
		...
		<view class="pay-content">
			...
			<view class="pay-content-mid">
				<view v-if="availableBalance >= deposit" class="pay-content-mid-item" @click="payType(1)">
					<view class="pay-content-mid-item-left">
						...
						<view class="pay-content-mid-item-left-money"><text style="margin-right: 10rpx; font-size: 24rpx;">余额</text><text>{{ parseFloat(availableBalance).toFixed(2) }}</text></view>
					</view>
				...
				<view class="pay-content-mid-item" style="border: 0;" @click="payType(3)">
					<view class="pay-content-mid-item-left">
			...
			<view class="logoutCon4">
				...
				<view class="logoutAgreeTit">确认支付即视为您已阅读并同意<text style="color: #F96F2B;" @click="goPact(1)">《货物运输交易规则》</text><text style="color: #F96F2B;" @click="goPact(2)">《货物运输协议》</text></view>
			</view>
		</view>
		<view class="payBtn">
	...
	export default {
		data() {
			return {
				...
				// 支付类别
				payTypes: 3,  // 1表示钱包支付，2表示微信支付，3表示支付宝支付
				...
				ajax.post(config.order_url,{
					...
					if(res.code == 0){
						if($paymentMethod == 'ALY_PAY') {
							...
							setTimeout(() => {
								uni.reLaunch({
									url: "/pages/orderDetails/orderDetails?id=" + res.data.freightShippingOrderDTO.id
								})
								uni.hideLoading();
							},2000)
						}
