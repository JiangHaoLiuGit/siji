<template>
	<view class="orderDetails">
		...
		<view class="orderCont" :style="orderDetails.orderState == 1 || orderDetails.orderState == 2 || orderDetails.orderState == 5 ? magBot : ''">
			...
			<view class="orderCont-tit">
				...
				<view class="orderCont-tit-left" v-if="orderDetails.orderState == 5 || orderDetails.orderState == 6">
					<view class="orderCont-tit-left-top">运输中</view>
			...
			<view class="orderCont-load">
				<view class="orderCont-load-top">
					<view class="orderCont-load-top-rig" v-for="(item,index) of loadSite" :key="item.cityName">
					...
					<view class="orderCont-load-top-rigss" v-for="(item,index) of unloadSite" :key="item.cityName">
						<view class="orderCont-load-top-rig-left">
			...
			<view class="orderCont-freight" v-if="orderDetails.loadingImagePath != null || orderDetails.unLoadingImagePath != null || orderDetails.receiptImagePath != null || orderDetails.orderState == 5">
				...
				<view class="orderCont-voucher-cont">
					...
					<view class="orderCont-voucher-cont-item" v-if="orderDetails.orderFreightCargoDTO.paymentMethod == 2">
		...
		<view class="btnStyle" v-if="orderDetails.orderState == 1 || orderDetails.orderState == 2 || orderDetails.orderState == 5">
			...
			<button v-if="orderDetails.orderState == 5 && orderDetails.orderFreightCargoDTO.paymentMethod == 2"
			 :disabled="orderDetails.loadingImagePath == null || orderDetails.unLoadingImagePath == null || orderDetails.receiptImagePath == null || btnDisabled" style="background: #FFC24D;" class="submitBtn"
			 :class="{loginButton: orderDetails.loadingImagePath == null || orderDetails.unLoadingImagePath == null || orderDetails.receiptImagePath == null || btnDisabled}" @click="goUnpaid">运输完成</button>
			 
			<button v-if="orderDetails.orderState == 5 && orderDetails.orderFreightCargoDTO.paymentMethod == 1"
			 :disabled="orderDetails.loadingImagePath == null || orderDetails.unLoadingImagePath == null || btnDisabled" style="background: #FFC24D;" class="submitBtn" :class="{loginButton: orderDetails.loadingImagePath == null || orderDetails.unLoadingImagePath == null || btnDisabled}"
			 @click="goUnpaid">运输完成</button>
		</view>
		...
		methods: {
			...
			getorderDetails() {
				ajax.get(config.order_url + `/${this.orderId}`, {}).then(res => {
					...
					console.log("获取获取订单详情数据返回失败 error is :" + err);
				})
			},
